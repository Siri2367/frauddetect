...
const transporter = nodemailer.createTransport({
  ...
  tls: {
    rejectUnauthorized: false  // College wifi certificate bypass
  },
  family: 4
});
  ...
  try {
    ...
    otpStore[email] = {
      ...
      expiresAt: Date.now() + 5 * 60 * 1000,
        ...
        // Fast response - user wait avadu
    res.json({ success: true });

    await transporter.sendMail({
      from: `"FraudDetect - Secure" <${process.env.EMAIL_USER}>`,
      to: email,
      subject: `Your verification code is ${otp}`,
      text: `Your verification code is ${otp}. Valid for 5 minutes.`,
      html: `
        <div style="font-family: Arial; max-width: 500px; margin: auto; border: 1px solid #ddd; border-radius: 10px; padding: 20px;">
          <h2 style="text-align: center;">Verification Code</h2>
          <h1 style="text-align: center; background: #f4f4f4; padding: 15px; letter-spacing: 5px;">${otp}</h1>
          <p>Valid for 5 minutes. If you didn't request this, ignore it.</p>
        </div>
      `,
    });
